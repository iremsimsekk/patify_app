...
class _ShelterDashboardScreenState extends State<ShelterDashboardScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final dashboardContent = Scaffold(
      appBar: AppBar(
        title:
            Text(widget.shelterUser.name, style: const TextStyle(fontSize: 18)),
              ...
              TextButton.icon(
                onPressed: () async {
                  final result = await Navigator.push(
                    ...
                    MaterialPageRoute(
                        builder: (_) =>
                            AddAnimalScreen(shelterUser: widget.shelterUser)),
                  );
                  if (result == true) {
                    _refreshList(); // Ekleme yapıldıysa listeyi yenile
                  }
                },
    ...
    final profileContent =
        ShelterProfileScreen(shelterUser: widget.shelterUser);
